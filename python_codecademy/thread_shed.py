...
daily_transactions_split = [tran.split("_") for tran in daily_transactions]
...
# transactions_clean = [detail.strip() for tran in daily_transactions_split for detail in tran]
transactions_clean = []
for tran in daily_transactions_split:
    detail_clean = []
    for detail in tran:
        detail_clean.append(detail.strip())
    transactions_clean.append(detail_clean)
...
# customers, sales, thread_sold = [detail[0], detail[1], detail[2] for detail in transactions_clean]
customers = []
sales = []
thread_sold = []
for detail in transactions_clean:
    customers.append(detail[0])
    sales.append(detail[1])
    thread_sold.append(detail[2])
...
for color in thread_sold:
    if "&" in color:
        multi_color = color.split("&")
        for single_color in multi_color:
...
# Now, using the list colors, the string method .format(), and the function color_count, iterate through colors list and print a sentence that says how many threads of each color were sold today.
for color in colors:
    print("The '{}' color has total {} threads sold.".format(color, color_count(color)))
